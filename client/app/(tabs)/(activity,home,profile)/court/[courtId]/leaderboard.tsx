...
import { getCourtActivePlayers, getCourtLeaderboard } from '@/lib/endpoints';
...
export default function CourtLeaderboardPage() {
  ...
  const { data: users, isPending } = useQuery({
    queryFn: async () => getCourtLeaderboard(courtId),
    queryKey: ['court', courtId, 'leaderboard'],
  });
          ...
          <NativewindFlatList
            ...
            renderItem={({ item }) => (
              <UserItem className="flex-1" user={item}>
                <Text className="text-lg font-bold">#{item.rank}</Text>
              </UserItem>
            )}
